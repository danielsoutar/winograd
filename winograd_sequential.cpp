 ...
 * filter to do convolutions with, and returning a Cx4x4 output, where C is the
 * number of input channels.
 *
 * In our case, we take the c-th 3x3 input feature map. This is done in
 * row-major order.
 */
std::vector<float> transform_filter(std::vector<float> filter, int channels,
                                    int c) {
  std::vector<float> tile(4 * 4 * channels);
...
#undef M
  ...
  Tensor conv2d(Tensor inp, Tensor fil, Conv2DParamPack conv2d_params) {
    ...
    for (int i = 0; i < T; ++i)
      for (int j = 0; j < E; ++j)
        M[i * E + j] = transformed_inputs[i][j] * transformed_filter[0][j];

  ...
  for(int channel = 0; channel < channels; ++channel) {
    ...
    for(int input_i = r; input_i < r + tile_size; ++input_i) {
      for(int input_j = c; input_j < c + tile_size; ++input_j) {
        auto idx = channel_tile_offset + (input_i - r) * tile_size * channels + (input_j - c) * channels;
  ...
  for(int c = 0; c < channels; ++c) {
    for(int i = 0; i < tile_size; ++i) {
      for(int j = 0; j < tile_size; ++j) {
        std::cout << tile[c + i*tile_size*channels + j*channels] << " ";
      }
      std::cout << "\n";
    }
    std::cout << "\n";
...
#ifndef TILE
#define TILE(X) tile[X * tile_size * tile_size]
#ifndef g
#define g(X) filter[X]
  for(int c = 0; c < channels; ++c) {
    float g0_g2 = (g(0) + g(2));
...
std::vector<float> inverse_transform(std::vector<float> M, int out_rows, int out_cols, int n_tiles_rows) {
...
#define M(I) M[t_i*16 + I]
  for(int r = 0; r < out_rows; r += 2) {
    for(int i = 0; i < n_tiles_rows; ++i) {
      Y[r*out_cols + i*2] = M(0) + M(1) + M(2) + M(4) + M(5) + M(6) + M(8) + M(9) + M(10);
      Y[r*out_cols + i*2 + 1] = M(1) + M(5) + M(9) - M(2) - M(6) - M(10) - M(3) - M(7) - M(11);
      Y[(r+1)*out_cols + i*2] = M(4) + M(5) + M(6) - M(8) - M(9) - M(10) - M(12) - M(13) - M(14);
      Y[(r+1)*out_cols + i*2 + 1] = M(5) - M(9) - M(13) - M(6) + M(10) + M(14) - M(7) + M(11) + M(15);
...
Tensor conv2d(Tensor inp, Tensor fil, Conv2DParamPack conv2d_params) {
  ...
  auto n_tiles_rows = conv2d_params_for_tiles.get_out_sizes(inp.shape(), tile_size)[2];
  auto n_tiles_cols = n_tiles_rows;
  int T = n_tiles_rows * n_tiles_cols;

  std::cout << "Number of tiles per row: " << n_tiles_rows << ", number of tiles per col: " << n_tiles_cols << "\n";
  ...
  // We want an input of shape [T, E, N, C], where E denotes the number of elements in the intermediate
  // tensor to do a pointwise multiply, and T the total number of tiles.
  std::vector<std::vector<float>> transformed_inputs(n_tiles_rows * n_tiles_cols);
  for(int i = 0; i < n_tiles_rows; ++i) {
    for(int j = 0; j < n_tiles_cols; ++j) {
      transformed_inputs[i*n_tiles_cols + j] = transform_input(inp.data(), fil.shape()[1], inp.shape()[2], inp.shape()[3], conv2d_params.padding(), i, j);
    }
  }
  return out;

...
struct TensorSizeStruct {
  public:
    ...
    TensorSizeStruct(std::vector<unsigned int> s) {
      shape = s;
    }
...
class Conv2DParamPack {
  public:
    ...
    TensorSizeStruct get_out_sizes(TensorSizeStruct inp_sz, TensorSizeStruct fil_sz) {
      unsigned int out_size =
        std::floor((inp_sz[2] + 2 * padding_ - fil_sz[2]) / stride_) + 1;
      std::vector<unsigned int> out_shape
        = {inp_sz[0], fil_sz[0], out_size, out_size};
      return out_shape;
    }
        ...
        for(int c = 0; c < dims[1]; ++c) {
          for(int h = 0; h < dims[2]; ++h) {
            for(int w = 0; w < dims[3]; ++w)
              std::cout << data_[n * dims[0] + c * dims[1] + h * dims[2] + w] << " ";
            std::cout << "\n";
          }
          std::cout << "\n";
        }
